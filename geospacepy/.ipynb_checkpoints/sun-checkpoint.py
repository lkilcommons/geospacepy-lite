...
def local_hour_angle(jds,glons):
    ...
    .. note::
        ...
        0 rad when sun is directly overhead. 
    ...
    # Wrap for lhas to be between [-pi,pi]
    lhas = np.mod(lhas + pi, 2*pi) - pi;
    return lhas
...
def local_mean_solar_time(jds,glons):
    ...
    2.) pi (hour 12) when the sun is directly overhead (noon), and 0/2pi (hour 0/24) 
        at midnight. 
    
    Convert to hours multiplying by 12/pi.
